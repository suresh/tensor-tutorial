...
def maybe_download():
    """Downlods training data and returns train and test file names."""
    if FLAGS.train_data:
...
def build_estimator(model_dir):
    ...
    wide_columns = [gender, native_country, education, occupation, workclass, relationship, age_buckets,
                    tf.contrib.layers.crossed_column([education, occupation], hash_bucket_size=10000),
                    tf.contrib.layers.crossed_column([age_buckets, education, occupation], hash_bucket_size=10000),
                    tf.contrib.layers.crossed_column([native_country, occupation], hash_bucket_size=10000)]
    ...
    if FLAGS.model_type == "wide":
        m = tf.contrib.learn.LinearClassififer(model_dir=model_dir, feature_columns=wide_columns)
...
def input_fn(df):
    ...
    feature_cols = dict(continous_cols)
...
def train_and_eval():
    ...
    # remove NaN elements from the dataset
